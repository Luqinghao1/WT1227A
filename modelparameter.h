 ...
 * 功能描述:
 * 1. 管理项目核心数据和文件交互。
 * 2. [新增逻辑] savePlottingData 现在会将绘图数据写入单独的 "项目名-date.json" 文件中。
 * 3. [新增逻辑] loadProject 会自动尝试读取同目录下的 "项目名-date.json" 文件以恢复图表数据。
 */
...
public:
    ...
    // 加载项目文件 (.pwt)
    // 同时会自动寻找并加载同名的 "-date.json" 图表数据文件
    ...
    // [修改] 保存绘图数据
    // 将 plots 数据保存到单独的 "[项目名]-date.json" 文件中
...
private:
    ...
    // 辅助：获取图表数据文件的路径
    // 例如: D:/proj/demo.pwt -> D:/proj/demo-date.json
